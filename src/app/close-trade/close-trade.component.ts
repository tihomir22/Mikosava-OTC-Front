...
export class CloseTradeComponent {
  ...
  public getURL = () =>
    window.location.origin + '/trade/' + this.viewedTrade.tradeId;

  public getAccount = of(getTools(this.store)).pipe(
    switchMap((tools) => {
      return tools;
    }),
    map((tools) => {
  ...
  public async exchange() {
    try {
      let [provider, signer, account, foundActiveNetwork] = await getTools(
        this.store
      );
  ...
  public async approve() {
    const [, signer, , foundActiveNetwork] = await getTools(this.store);
